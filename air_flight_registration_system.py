...
def home():
	if(session):
		if session['type'] == 'Customer':
			...
			return render_template('homeAgt.html')
...
def registerAuth():
	...
	if registertype == 'Customer':
		...
		query = "SELECT * FROM agent WHERE username = '{}'"
	elif registertype == 'Airline Staff':
	...
	if(data):
		...
		if registertype == 'Customer':
			...
			ins = "INSERT INTO airline_staff(username, password) VALUES('{}', '{}')"
...
def registerStf():
    ...
    # cursor = conn.cursor()
    # query = "SELECT * FROM flight ORDER BY flight_num DESC"
...
def search():
	...
	cursor.close()
	if(data):
		return render_template('search_result.html', result = data, error = None)
	else:
...
def viewCus():
	...
	if action == 'View My Flight':
		...
		data = cursor.fetchall()
		cursor.close()
		return render_template('homeCus.html', result = data)
...
def searchCus():
	...
	cursor.close()
	if(data):
		return render_template('homeCus.html', result1 = data, error = None)
	else:
		error = "No result found."
		return render_template('homeCus.html', error = error)
...
def purticketCus():
	...
	conn.commit()
	cursor.close()
	return render_template('homeCus.html', banner = "Purchase Success")
...
def chartCus():
	...
	ticket WHERE customer_email = '{}' AND DATE(depart_time) > '2022-06-14' AND DATE(depart_time) < '2022-12-13' \
	GROUP BY month(depart_time)"
	query1 = "SELECT sum(price) AS spent FROM flight NATURAL JOIN \
	ticket WHERE customer_email = '{}' AND DATE(depart_time) > '2021-12-14' AND DATE(depart_time) < '2022-12-13'"
	cursor = conn.cursor()
	cursor.execute(query.format(username))
	data = cursor.fetchall()
	cursor.close()
...
def viewAgt():
	...
	if action == 'View My Flight':
		...
		data = cursor.fetchall()
		cursor.close()
		return render_template('homeAgt.html', result = data)
...
def searchAgt():
	...
	if(data):
		...
		return render_template('homeAgt.html', result1 = data, error = None, airline = data2)
	else:
		error = "No result found."
		return render_template('homeAgt.html', error = error)
...
def purticketAgt():
	...
	if (data):
		...
		conn.commit()
		cursor.close()
		return render_template('homeAgt.html', banner = "Purchase Success")
	else:
		banner = 'Customer email does not exist.'
		cursor.close()
		return render_template('homeAgt.html', banner = banner)
...
def view_comm():
	...
	username = session['username']
	query = "SELECT ifnull(sum(price), 0), ifnull(avg(price), 0), count(*) FROM flight NATURAL JOIN ticket WHERE agent_email = '{}' AND\
	 DATE(depart_time) > '{}' AND DATE(depart_time) < '{}'" 
	cursor = conn.cursor()
	cursor.execute(query.format(username, start, end))
	data = cursor.fetchall()
	error = None
	cursor.close()
	if(data):
		return render_template('homeAgt.html', result1 = data, error = None)
	else:
		error = "No result found."
		return render_template('homeAgt.html', error = error)
...
def viewStf():
	...
	if action == 'View My Flight':
		query = 'SELECT flight_num, depart_time, arrive_time FROM flight WHERE name_airline = "{}" AND\
		 DATE(depart_time) < "2023-01-13"'
		cursor = conn.cursor()
		cursor.execute(query.format(airline))
		data = cursor.fetchall()
		cursor.close()
		return render_template('homeStf.html', result = data)
...
def viewFlight():
	...
	conn.commit()
	cursor.close()
	return render_template('homeStf.html', result1 = result)
...
def tocreate():
	...
	if(permission == 1 or permission == 3):
		...
		return render_template('homeStf.html', error = "You are not Admin")
...
def showTable():
	...
	query = "SELECT flight_num, depart_time, arrive_time FROM flight WHERE name_airline = '{}' AND\
	 DATE(depart_time) < '2023-01-13' AND status = 'upcoming'"
	cursor = conn.cursor()
...
def toupdate():
	...
	if(permission == 1 or permission == 2):
		...
		return render_template('homeStf.html', error = "You are not Operator")
...
# @app.route('/showPlane', methods = ['GET', 'POST'])
# def showPlane():
# 	username = session['username']
# 	name_airline = session['airline']
# 	query = "SELECT * FROM airplane WHERE name_airline = '{}'"
# 	cursor = conn.cursor()
# 	cursor.execute(query.format(name_airline))
# 	data = cursor.fetchall()
# 	cursor.close()
# 	return render_template('addPlane.html', result = data)
...
def viewAgents():
	...
	JOIN flight f USING(flight_num) WHERE f.name_airline = '{}' AND DATE(f.depart_time) > '2022-11-14' AND DATE(f.arrive_time) < '2022-12-13' \
	...
	JOIN flight f USING(flight_num) WHERE f.name_airline = '{}' AND DATE(f.depart_time) > '2021-12-14' AND DATE(f.arrive_time) < '2022-12-13' \
	GROUP BY t.agent_email ORDER BY count(t.ticket_id) DESC LIMIT 5"
	...
	cursor3.close()
	return render_template('homeStf.html', result1 = data1, result2 = data2, result3 = data3)
...
def viewFreqCus():
	...
	query1 = "SELECT c.email, c.name FROM flight f JOIN ticket t using(flight_num) join customer c on c.email = t.customer_email\
			  WHERE f.name_airline = '{}' AND DATE(f.depart_time) > '2021-12-14' AND DATE(f.arrive_time) < '2022-12-13' GROUP BY t.customer_email ORDER BY count(t.ticket_id) DESC LIMIT 1 "
	cursor1 = conn.cursor()
	cursor1.execute(query1.format(name_airline))
	data1 = cursor1.fetchone()
	cursor1.close()
	...
	data2 = cursor2.fetchall()
	cursor2.close()
	return render_template('homeStf.html', result_cus = data1, result_fli = data2)
...
def ticketReports():
	...
	airline = session['airline']
	query = "SELECT month(depart_time) AS month, count(ticket_id) AS monthly_sold FROM flight NATURAL JOIN \
	ticket WHERE name_airline = '{}' AND DATE(depart_time) > '{}' AND DATE(depart_time) < '{}' \
	GROUP BY month(depart_time)"
	cursor = conn.cursor()
...
def revenueComparisonMonth():
	name_airline = session['airline']
	query1 = "SELECT ifnull(sum(price), 0) FROM ticket NATURAL JOIN flight WHERE agent_email is NULL AND name_airline = '{}' AND \
	DATE(depart_time) > '2022-11-14' AND DATE(depart_time) < '2022-12-13' "
	query2 = "SELECT ifnull(sum(price), 0) FROM ticket NATURAL JOIN flight WHERE agent_email is not NULL AND name_airline = '{}' AND \
	DATE(depart_time) > '2022-11-14' AND DATE(depart_time) < '2022-12-13'"
	# query3 = "SELECT ifnull(sum(price), 0) FROM ticket NATURAL JOIN flight WHERE agent_email is NULL AND name_airline = '{}' AND \
	# DATE(depart_time) > '2021-12-14' AND DATE(depart_time) < '2022-12-13' "
	# query4 = "SELECT ifnull(sum(price), 0) FROM ticket NATURAL JOIN flight WHERE agent_email is not NULL AND name_airline = '{}' AND \
	# DATE(depart_time) > '2021-12-14' AND DATE(depart_time) < '2022-12-13'"
	cursor1 = conn.cursor()
	cursor1.execute(query1.format(name_airline))
...
# @app.route('/gobackhomeStf')
# def gobackhomeStf():
#  	render_template('homeStf.html', error = "Welcome back.")
...
def viewReports():
	...
	ON f.flight_num = t.flight_num WHERE f.name_airline = '{}' AND DATE(f.depart_time) > '2022-09-14' AND DATE(f.arrive_time) \
	< '2022-12-13' GROUP BY a.city ORDER BY count(t.ticket_id) DESC LIMIT 3"
	query2 = "SELECT a.city, count(t.ticket_id) FROM (airport a JOIN flight f ON f.arrive_airport = a.name) RIGHT JOIN ticket t \
	ON f.flight_num = t.flight_num WHERE f.name_airline = '{}' AND DATE(f.depart_time) > '2021-12-14' AND DATE(f.arrive_time) \
	< '2022-12-13' GROUP BY a.city ORDER BY count(t.ticket_id) DESC LIMIT 3"
	cursor1 = conn.cursor()
	cursor1.execute(query1.format(name_airline))
	data1 = cursor1.fetchall()
	...
	data2 = cursor2.fetchall()
	cursor2.close()
	return render_template('homeStf.html', result_des_3 = data1, result_des_y = data2)
...
def toAddAgents():
	permission = session['permission']
	if(permission == 1 or permission == 3):
		#return redirect(url_for('addAgents'))
